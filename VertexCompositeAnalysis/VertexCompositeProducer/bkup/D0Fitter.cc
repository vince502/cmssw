...
//      TransientTrack tmpTk( *tmpRef, &(*bFieldHandle), globTkGeomHandle );
  ...
  // Since we don't know maxPt yet, we'll sort by pt and use early breaks instead
  
  // Create index vectors for sorting by pt (descending) for early break optimization
  ...
  std::sort(posIndices.begin(), posIndices.end(), 
            [&](size_t i, size_t j) { return theTrackRefsPos[i]->pt() > theTrackRefsPos[j]->pt(); });
  std::sort(negIndices.begin(), negIndices.end(), 
            [&](size_t i, size_t j) { return theTrackRefsNeg[i]->pt() > theTrackRefsNeg[j]->pt(); });
  ...
  if (!isWrongSign) {
    ...
    if (theTrackRefsPos.empty() || theTrackRefsNeg.empty()) return;
    ...
    for(unsigned int ipos = 0; ipos < posIndices.size(); ipos++) {
      ...
      // Early break: if this track's pt + smallest neg track pt < cut, no more valid pairs
      if (positiveTrackRef->pt() + theTrackRefsNeg[negIndices.back()]->pt() < tkPtSumCut) break;
      ...
      for(unsigned int ineg = 0; ineg < negIndices.size(); ineg++) {
        ...
        // Early break: if pt sum is too small, remaining tracks (smaller pt) won't work
        if( (positiveTrackRef->pt() + negativeTrackRef->pt()) < tkPtSumCut) break;
        if( abs(positiveTrackRef->eta() - negativeTrackRef->eta()) > tkEtaDiffCut) continue;
  ...
  // perform MVA evaluation
    ...
    if (theTrackRefsPos.size() >= 2) {
      for(unsigned int ipos1 = 0; ipos1 < posIndices.size(); ipos1++) {
        ...
        // Early break: if this track's pt + smallest remaining pos track pt < cut, no more valid pairs
        if (ipos1 + 1 < posIndices.size() && 
            positiveTrackRef->pt() + theTrackRefsPos[posIndices.back()]->pt() < tkPtSumCut) break;
        
        for(unsigned int ipos2 = ipos1 + 1; ipos2 < posIndices.size(); ipos2++) {
          size_t itrkpos2 = posIndices[ipos2];
          TrackRef negativeTrackRef = theTrackRefsPos[itrkpos2]; // Wrong-sign: both are positive
        ...
        //This vector holds the pair of same-charged tracks to be vertexed (wrong-sign)
        std::vector<TransientTrack> transTracks;
    ...
    if (theTrackRefsNeg.size() >= 2) {
      for(unsigned int ineg1 = 0; ineg1 < negIndices.size(); ineg1++) {
        ...
        TrackRef positiveTrackRef = theTrackRefsNeg[itrkneg1]; // Wrong-sign: both are negative
        TransientTrack* posTransTkPtr = &theTransTracksNeg[itrkneg1];
        
        // Early break: if this track's pt + smallest remaining neg track pt < cut, no more valid pairs
        if (ineg1 + 1 < negIndices.size() && 
            positiveTrackRef->pt() + theTrackRefsNeg[negIndices.back()]->pt() < tkPtSumCut) break;
        
        for(unsigned int ineg2 = ineg1 + 1; ineg2 < negIndices.size(); ineg2++) {
