    ...
    bField_esToken_(iC.esConsumes<MagneticField, IdealMagneticFieldRecord>())
{
  ...
  // Input collections - unified dilepton input (can be dimuon or dielectron)
...
// Detect lepton channel from dilepton candidate
BDiLeptonFitter::LeptonChannel BDiLeptonFitter::detectLeptonChannel(const pat::CompositeCandidate& dilepton) {
  // Try muon channel first (HiOnia2MuMuPAT uses "muon1", "muon2")
  if (dilepton.daughter("muon1") != nullptr && dilepton.daughter("muon2") != nullptr) {
    const pat::Muon* mu1 = dynamic_cast<const pat::Muon*>(dilepton.daughter("muon1"));
    const pat::Muon* mu2 = dynamic_cast<const pat::Muon*>(dilepton.daughter("muon2"));
    if (mu1 && mu2) {
      return LeptonChannel::Muon;
    }
  }
  
  // Try electron channel (HiOnia2EEPAT uses "electron1", "electron2")
  if (dilepton.daughter("electron1") != nullptr && dilepton.daughter("electron2") != nullptr) {
    const pat::Electron* e1 = dynamic_cast<const pat::Electron*>(dilepton.daughter("electron1"));
    const pat::Electron* e2 = dynamic_cast<const pat::Electron*>(dilepton.daughter("electron2"));
    if (e1 && e2) {
...
double BDiLeptonFitter::getLeptonMass(LeptonChannel channel) {
  switch (channel) {
    ...
    default: return muonMass; // fallback
  }
}
...
void BDiLeptonFitter::fitAll(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (!dileptonHandle.isValid() || !trackHandle.isValid() || !trackHandle->size()) {
    ...
    if(!dileptonHandle.isValid()) cout << "Dilepton!" << endl;
  ...
  for (const auto& dilepton : *dileptonHandle) {
...
#if DEBUG_BFIT == true
      cout << "Failed to get lepton tracks" << endl;
#endif
      continue;
    }
...
#if DEBUG_BFIT == true
    cout << "Processing dilepton with channel: " << (channel == LeptonChannel::Muon ? "Muon" : "Electron") << endl;
#endif
    ...
    if (doBPlus) {
      for (size_t i = 0; i < selectedTracks.size(); i++) {
        ...
        // Skip tracks that are daughters of the dilepton
        ...
        // Get dilepton daughters as transient tracks
        std::vector<reco::TransientTrack> bTracks;
        std::vector<double> bMasses;
 
#if DEBUG_BFIT == true
        cout << "Dilepton basic property (pt, y): " << dilepton.pt() << ", "<< dilepton.y() << " with Ndau " << dilepton.numberOfDaughters() << endl;
#endif
...
pat::CompositeCandidate BDiLeptonFitter::createBPlus(const pat::CompositeCandidate& dilepton,
  ...
  // Set the dilepton as first daughter
  ...
  // Store lepton channel information
