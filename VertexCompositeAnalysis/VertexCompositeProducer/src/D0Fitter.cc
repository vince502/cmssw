...
namespace {
float safeMVAInput(float value) {
...
D0Fitter::D0Fitter(const edm::ParameterSet &theParameters,
  ...
  if (theParameters.exists("mvaType"))
    type = theParameters.getParameter<std::string>("mvaType");
  mvaType_ = type;
  useTMVA_ = useAnyMVA_ && (mvaType_ == "TMVA");
  useONNX_ = useAnyMVA_ && !useTMVA_;

  if (useTMVA_) {
    tmvaWeightsFile_ = "TMVAClassification_BDT.weights.xml";
    if (theParameters.exists("tmvaWeightsFile"))
      tmvaWeightsFile_ = theParameters.getParameter<std::string>("tmvaWeightsFile");

    edm::FileInPath weightFile(
        "VertexCompositeAnalysis/VertexCompositeProducer/data/" + tmvaWeightsFile_);
    TMVA::Tools::Instance();
    tmvaReader_ = std::make_unique<TMVA::Reader>("!Color:!Silent");
    tmvaReader_->AddVariable("v_2DCosPointingAngle", &tmvaV2dCosPointingAngle_);
    tmvaReader_->AddVariable("v_3DCosPointingAngle", &tmvaV3dCosPointingAngle_);
    tmvaReader_->AddVariable("v_2DDecayLengthSignificance", &tmvaV2dDecayLengthSignificance_);
    tmvaReader_->AddVariable("v_3DDecayLengthSignificance", &tmvaV3dDecayLengthSignificance_);
    tmvaReader_->AddVariable("v_2DDecayLength", &tmvaV2dDecayLength_);
    tmvaReader_->AddVariable("v_3DDecayLength", &tmvaV3dDecayLength_);
    tmvaReader_->AddVariable("v_2DPointingAngle", &tmvaV2dPointingAngle_);
    tmvaReader_->AddVariable("v_3DPointingAngle", &tmvaV3dPointingAngle_);
    tmvaReader_->AddVariable("pT", &tmvaPt_);
    tmvaReader_->AddVariable("eta", &tmvaEta_);
    tmvaReader_->AddVariable("NHitD1", &tmvaNHitD1_);
    tmvaReader_->AddVariable("NHitD2", &tmvaNHitD2_);
    tmvaReader_->AddVariable("pTD1", &tmvaPtD1_);
    tmvaReader_->AddVariable("pTD2", &tmvaPtD2_);
    tmvaReader_->AddVariable("EtaD1", &tmvaEtaD1_);
    tmvaReader_->AddVariable("EtaD2", &tmvaEtaD2_);
    tmvaReader_->AddVariable("derived_dau_abs_eta_gap", &tmvaDerivedDauAbsEtaGap_);
    tmvaReader_->AddVariable("derived_dau_pt_imbalance", &tmvaDerivedDauPtImbalance_);
    tmvaReader_->AddSpectator("mass", &tmvaSpecMass_);
    tmvaReader_->AddSpectator("pt", &tmvaSpecPt_);
    tmvaReader_->AddSpectator("rapidity", &tmvaSpecRapidity_);
    tmvaReader_->AddSpectator("eta", &tmvaSpecEta_);
    tmvaReader_->AddSpectator("flavor", &tmvaSpecFlavor_);
    tmvaReader_->AddSpectator("match_gen", &tmvaSpecMatchGen_);
    tmvaReader_->AddSpectator("is_swap", &tmvaSpecIsSwap_);
    tmvaReader_->AddSpectator("ancestor_flavor", &tmvaSpecAncestorFlavor_);
    tmvaReader_->AddSpectator("is_data_sideband", &tmvaSpecIsDataSideband_);
    tmvaReader_->AddSpectator("source_tree", &tmvaSpecSourceTree_);
    const std::string weightPath = weightFile.fullPath();
    tmvaReader_->BookMVA("BDT", weightPath.c_str());
  }
  ...
  if (useONNX_) {
    if (theParameters.exists("input_names") &&
      ...
      throw cms::Exception("Configuration")
          << "input_names/output_names not provided for ONNX D0 MVA";
    }
...
float D0Fitter::evaluateTMVA(float v2dCosPointingAngle,
                             ...
                             float etaD1,
                             float etaD2) {
  const float dauPtSum = ptD1 + ptD2;
  if (!tmvaReader_ || !(dauPtSum > 0.0f) || !std::isfinite(dauPtSum))
    return -999.0f;

  tmvaV2dCosPointingAngle_ = safeMVAInput(v2dCosPointingAngle);
  tmvaV3dCosPointingAngle_ = safeMVAInput(v3dCosPointingAngle);
  tmvaV2dDecayLengthSignificance_ = safeMVAInput(v2dDecayLengthSignificance);
  tmvaV3dDecayLengthSignificance_ = safeMVAInput(v3dDecayLengthSignificance);
  tmvaV2dDecayLength_ = safeMVAInput(v2dDecayLength);
  tmvaV3dDecayLength_ = safeMVAInput(v3dDecayLength);
  tmvaV2dPointingAngle_ = safeMVAInput(v2dPointingAngle);
  tmvaV3dPointingAngle_ = safeMVAInput(v3dPointingAngle);
  tmvaPt_ = safeMVAInput(pt);
  tmvaEta_ = safeMVAInput(eta);
  tmvaNHitD1_ = safeMVAInput(nHitD1);
  tmvaNHitD2_ = safeMVAInput(nHitD2);
  tmvaPtD1_ = safeMVAInput(ptD1);
  tmvaPtD2_ = safeMVAInput(ptD2);
  tmvaEtaD1_ = safeMVAInput(etaD1);
  tmvaEtaD2_ = safeMVAInput(etaD2);
  tmvaDerivedDauAbsEtaGap_ = safeMVAInput(std::abs(etaD1 - etaD2));
  tmvaDerivedDauPtImbalance_ = safeMVAInput(std::abs((ptD1 - ptD2) / dauPtSum));
  tmvaSpecPt_ = tmvaPt_;
  tmvaSpecEta_ = tmvaEta_;

  return safeMVAInput(tmvaReader_->EvaluateMVA("BDT"));
}

/**
 * @brief Kinematic pre-filter for D0 -> K pi candidates
 * Uses geometric mean scaling (2*sqrt(pt1*pt2)) for pT-unbiased selection.
 * Optimized: 30%+ eff at low pT, 71% at high pT, 5.9x bkg rejection.
 */
bool passD0KinematicPreFilter(const reco::TrackRef &tk1,
                              const reco::TrackRef &tk2) {
  const double pt1 = tk1->pt();
  const double pt2 = tk2->pt();
  // Geometric mean scaling for asymmetric decays
  const double geomPT = 2.0 * std::sqrt(std::max(pt1 * pt2, 0.0001));

  // Delta R filter
  const double dr = reco::deltaR(*tk1, *tk2);
  const double drMax = (4.34 / geomPT) + 0.28;
  if (dr > drMax)
    return false;

  // Delta Phi filter
  const double dphi = std::abs(reco::deltaPhi(tk1->phi(), tk2->phi()));
  const double dphiMax = (3.15 / geomPT) + 0.21;
  if (dphi > dphiMax)
    return false;

  // Eta Gap filter
  const double deta = std::abs(tk1->eta() - tk2->eta());
  const double detaMax = (2.80 / geomPT) + 0.21;
  if (deta > detaMax)
    return false;
...
void D0Fitter::fitAll(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  double time_trackSel_ms = 0, time_combination_ms = 0, time_fitting_ms = 0,
         time_bdt_ms = 0;
  long n_pairs_checked = 0, n_pairs_passed_prefilter = 0, n_fits = 0,
       n_bdt_evals = 0;
  ...
  for (unsigned int indx = 0; indx < theTrackHandle->size(); indx++) {
    ...
    auto t_after_tracksel = std::chrono::high_resolution_clock::now();
    time_trackSel_ms =
        std::chrono::duration<double, std::milli>(t_after_tracksel - t_start)
            .count();
    ...
    if (!isWrongSign) {
      ...
      for (unsigned int ipos = 0; ipos < posIndices.size(); ipos++) {
        ...
        // Early break: if this track's pt + smallest neg track pt < cut, no
        // more valid pairs
        ...
        for (unsigned int ineg = 0; ineg < negIndices.size(); ineg++) {
          ...
          n_pairs_checked++;
          auto t_comb_start = std::chrono::high_resolution_clock::now();

          // Kinematic pre-filter based on signal envelope
          if (!passD0KinematicPreFilter(positiveTrackRef, negativeTrackRef))
            continue;

          n_pairs_passed_prefilter++;
          auto t_comb_end = std::chrono::high_resolution_clock::now();
          time_combination_ms += std::chrono::duration<double, std::milli>(
                                     t_comb_end - t_comb_start)
                                     .count();
          ...
          // math::XYZVector deltaP(pospT.x() - negpT.x(), pospT.y() -
          // negpT.y(), 0);
          ...
          for (int i = 0; i < 2; i++) {
            ...
            time_fitting_ms += std::chrono::duration<double, std::milli>(
                ...
                theD0->mass() > d0MassD0 - d0MassCut) //&&
            {
              ...
              // GBR METHOD END
              if (useONNX_ && onnxRuntime_) {
                // Prepare input data for new 17-feature BDT model
                // Features in order:
                // 0: VtxProb, 1: decayLengthSig3D, 2: alpha3D, 3: pTD1, 4:
                // pTD2, 5: etaD1, 6: etaD2, 7: pT, 8: y, 9: dca3D, 10:
                // trackDCA, 11: DeltaEta, 12: DeltaPhi, 13: DeltaR, 14:
                // pTRatio, 15: dca3DSig, 16: trackDCASig
                cms::Ort::FloatArrays data_;

                data_.emplace_back(17, 0);
                std::vector<float> &onnxVals_ = data_[0];

                // Helper: return 0 if NaN/Inf, otherwise return value
                ...
                // Run ONNX inference
                auto t_bdt_start = std::chrono::high_resolution_clock::now();
                std::vector<float> outputs = onnxRuntime_->run(
                    input_names_, data_, input_shapes_, output_names_)[1];
                auto t_bdt_end = std::chrono::high_resolution_clock::now();
                time_bdt_ms += std::chrono::duration<double, std::milli>(
                                   t_bdt_end - t_bdt_start)
                                   .count();
                n_bdt_evals++;
              ...
              //   }
              // }
              else if (useTMVA_ && tmvaReader_) {
                const float tmvaVal = evaluateTMVA(
                    std::cos(d0Angle2D), std::cos(d0Angle3D),
                    rVtxMag / sigmaRvtxMag, lVtxMag / sigmaLvtxMag,
                    rVtxMag, lVtxMag, d0Angle2D, d0Angle3D, theD0->pt(),
                    theD0->eta(), positiveTrackRef->numberOfValidHits(),
                    negativeTrackRef->numberOfValidHits(), posCandTotalP.perp(),
                    negCandTotalP.perp(), posCandTotalP.eta(), negCandTotalP.eta());
                if (tmvaVal > mvaCut) {
                  theD0->addUserFloat("mva", tmvaVal);
                  mvaVals_.push_back(tmvaVal);
                  theD0s.push_back(std::move(*theD0));
                }
              } else {
                theD0s.push_back(*theD0);
      ...
      if (theTrackRefsPos.size() >= 2) {
        for (unsigned int ipos1 = 0; ipos1 < posIndices.size(); ipos1++) {
          ...
          if (ipos1 + 1 < posIndices.size() &&
              positiveTrackRef->pt() +
                      theTrackRefsPos[posIndices.back()]->pt() <
                  tkPtSumCut)
            break;
          ...
          for (unsigned int ipos2 = ipos1 + 1; ipos2 < posIndices.size();
            ...
            // Early break: if pt sum is too small, remaining tracks (smaller
            // pt) won't work
            if ((positiveTrackRef->pt() + negativeTrackRef->pt()) < tkPtSumCut)
              break;
            if (abs(positiveTrackRef->eta() - negativeTrackRef->eta()) >
            ...
            double totalE1 =
                sqrt(posTSCP.momentum().mag2() + kaonMassD0Squared) +
                sqrt(negTSCP.momentum().mag2() + piMassD0Squared);
            double totalE1Sq = totalE1 * totalE1;

            double totalE2 =
                sqrt(posTSCP.momentum().mag2() + piMassD0Squared) +
                sqrt(negTSCP.momentum().mag2() + kaonMassD0Squared);
            double totalE2Sq = totalE2 * totalE2;
            ...
            for (int i = 0; i < 2; i++) {
              ...
              GlobalVector posCandTotalP = GlobalVector(
                  posCandKP.momentum().x(), posCandKP.momentum().y(),
                  posCandKP.momentum().z());
              GlobalVector negCandTotalP = GlobalVector(
                  negCandKP.momentum().x(), negCandKP.momentum().y(),
                  negCandKP.momentum().z());
              ...
              if (d0NormalizedChi2 > chi2Cut || rVtxMag < rVtxCut ||
                  rVtxMag / sigmaRvtxMag < rVtxSigCut || lVtxMag < lVtxCut ||
                  lVtxMag / sigmaLvtxMag < lVtxSigCut ||
                  cos(d0Angle3D) < collinCut3D ||
                  cos(d0Angle2D) < collinCut2D || d0Angle3D > alphaCut ||
                  d0Angle2D > alpha2DCut)
                continue;
              AnalyticalImpactPointExtrapolator extrapolator(magField);
              TrajectoryStateOnSurface tsos = extrapolator.extrapolate(
                  d0Cand->currentState().freeTrajectoryState(),
              ...
              theD0->addUserFloat("decaylength2D", rVtxMag);
              theD0->addUserFloat("decaylength3D", lVtxMag);
              theD0->addUserFloat("decaylengthsignif2D",
                                  rVtxMag / sigmaRvtxMag);
              theD0->addUserFloat("decaylengthsignif3D",
                                  lVtxMag / sigmaLvtxMag);
              theD0->addUserFloat("dca3D", cur3DIP.value());
              theD0->addUserFloat("dca3DErr", cur3DIP.error());
              theD0->addUserFloat("track3DDCA", dca);
              theD0->addUserFloat("track3DDCAErr", dcaError);
              ...
              if (theD0->mass() < d0MassD0 + d0MassCut &&
                  theD0->mass() > d0MassD0 - d0MassCut) {
                if (useONNX_ && onnxRuntime_) {
                  // Same 17-feature BDT model as right-sign
                  cms::Ort::FloatArrays data_;
                  data_.emplace_back(17, 0);
                  std::vector<float> &onnxVals_ = data_[0];
                  ...
                  onnxVals_[1] = safe(lVtxMag / sigmaLvtxMag);
                  onnxVals_[2] = safe(d0Angle3D);
                  onnxVals_[3] = safe(pTD1);
                  onnxVals_[4] = safe(pTD2);
                  onnxVals_[5] = safe(etaD1);
                  onnxVals_[6] = safe(etaD2);
                  onnxVals_[7] = safe(theD0->pt());
                  onnxVals_[8] = safe(theD0->y());
                  onnxVals_[9] = safe(dca3D_val);
                  onnxVals_[10] = safe(dca);
                  onnxVals_[11] = safe(DeltaEta);
                  onnxVals_[12] = safe(DeltaPhi);
                  onnxVals_[13] = safe(DeltaR);
                  onnxVals_[14] = safe(pTRatio);
                  onnxVals_[15] = safe(dca3DSig);
                  onnxVals_[16] = safe(trackDCASig);

                  std::vector<float> outputs = onnxRuntime_->run(
                      input_names_, data_, input_shapes_, output_names_)[0];
      ...
      if (theTrackRefsNeg.size() >= 2) {
        for (unsigned int ineg1 = 0; ineg1 < negIndices.size(); ineg1++) {
          ...
          if (ineg1 + 1 < negIndices.size() &&
              positiveTrackRef->pt() +
                      theTrackRefsNeg[negIndices.back()]->pt() <
                  tkPtSumCut)
            break;
    ...
    //   // cout << input_names_.size() << " " << data_.size() << " " <<
    //   input_shapes_.size() << " " << output_names_.size() << endl; int nInput
    //   = data_[0].size()/VARSIZE; input_shapes_[0] ={nInput, VARSIZE}; auto
    //   fullOut = onnxRuntime_->run(input_names_, data_,
    //   input_shapes_,output_names_,nInput );
    ...
    //   // cout << "onnxRuntime_->run() time: " << duration.count() << "
    //   microseconds" << endl;
    // }
    // float onnxVal = outputs[1]; // Adjust if your model has multiple outputs

    // std::cout << "Total time for MVA: " << totaltime << " microseconds" <<
    // std::endl; std::cout << "Number of loops: " << nloop << std::endl;
    // std::cout << "Average time per loop: " << (totaltime/nloop) << "
    ...
    edm::LogInfo("D0Fitter")
        << "=== D0Fitter Timing Summary ===\n"
        << "  Total time: " << time_total_ms << " ms\n"
        << "  Track selection: " << time_trackSel_ms << " ms\n"
        << "  Combination/PreFilter: " << time_combination_ms << " ms\n"
        << "  Vertex fitting: " << time_fitting_ms << " ms\n"
        << "  BDT evaluation: " << time_bdt_ms << " ms\n"
        << "  Pairs checked: " << n_pairs_checked << "\n"
        << "  Pairs passed pre-filter: " << n_pairs_passed_prefilter << "\n"
        << "  Vertex fits: " << n_fits << "\n"
        << "  BDT evaluations: " << n_bdt_evals << "\n"
        << "  D0 candidates: " << theD0s.size();
  }
  // Get methods
