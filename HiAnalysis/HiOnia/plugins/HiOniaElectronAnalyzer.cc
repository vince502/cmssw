...
void HiOniaElectronAnalyzer::initTree() {
  ...
  // Lifetime and DCA
  tree_->Branch("Reco_ee_ctau", Reco_ee_ctau_, "Reco_ee_ctau[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_ctauErr", Reco_ee_ctauErr_, "Reco_ee_ctauErr[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_ctau3D", Reco_ee_ctau3D_, "Reco_ee_ctau3D[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_ctauErr3D", Reco_ee_ctauErr3D_, "Reco_ee_ctauErr3D[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_cosAlpha", Reco_ee_cosAlpha_, "Reco_ee_cosAlpha[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_cosAlpha3D", Reco_ee_cosAlpha3D_, "Reco_ee_cosAlpha3D[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_dca", Reco_ee_dca_, "Reco_ee_dca[Reco_ee_size]/F");
  
  // Vertex position
  tree_->Branch("Reco_ee_vtxX", Reco_ee_vtxX_, "Reco_ee_vtxX[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_vtxY", Reco_ee_vtxY_, "Reco_ee_vtxY[Reco_ee_size]/F");
  tree_->Branch("Reco_ee_vtxZ", Reco_ee_vtxZ_, "Reco_ee_vtxZ[Reco_ee_size]/F");
...
void HiOniaElectronAnalyzer::fillRecoDielectrons(const edm::Event& iEvent) {
  ...
  for (const auto& cand : *dielectrons) {
    ...
    // Lifetime and DCA (note: ppdlPV is stored as ctau = 10 * ppdl for consistency with mumu)
    Reco_ee_ctau_[idx] = cand.hasUserFloat("ppdlPV") ? 10.0f * cand.userFloat("ppdlPV") : kInvalidFloat;
    Reco_ee_ctauErr_[idx] = cand.hasUserFloat("ppdlErrPV") ? 10.0f * cand.userFloat("ppdlErrPV") : kInvalidFloat;
    Reco_ee_ctau3D_[idx] = cand.hasUserFloat("ppdlPV3D") ? 10.0f * cand.userFloat("ppdlPV3D") : kInvalidFloat;
    Reco_ee_ctauErr3D_[idx] = cand.hasUserFloat("ppdlErrPV3D") ? 10.0f * cand.userFloat("ppdlErrPV3D") : kInvalidFloat;
    Reco_ee_cosAlpha_[idx] = cand.hasUserFloat("cosAlpha") ? cand.userFloat("cosAlpha") : kInvalidFloat;
    Reco_ee_cosAlpha3D_[idx] = cand.hasUserFloat("cosAlpha3D") ? cand.userFloat("cosAlpha3D") : kInvalidFloat;
    Reco_ee_dca_[idx] = cand.hasUserFloat("DCA") ? cand.userFloat("DCA") : kInvalidFloat;
    
    // Vertex position from userData
    if (cand.hasUserData("PCAVtx")) {
      const reco::Vertex* vtx = cand.userData<reco::Vertex>("PCAVtx");
      Reco_ee_vtxX_[idx] = vtx->x();
      Reco_ee_vtxY_[idx] = vtx->y();
      Reco_ee_vtxZ_[idx] = vtx->z();
    } else {
      Reco_ee_vtxX_[idx] = kInvalidFloat;
      Reco_ee_vtxY_[idx] = kInvalidFloat;
      Reco_ee_vtxZ_[idx] = kInvalidFloat;
    }
