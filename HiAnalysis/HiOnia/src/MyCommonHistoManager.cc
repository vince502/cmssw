...
void MyCommonHistoManager::Add(std::string theAppendix, std::string theName2) {
  if (!hName.empty()) {
    if (!theName2.empty()) {
    ...
    if (ret.second == false) {
      edm::LogWarning("MyCommonHistograms")
          << "[MyCommonHistoManager::Add] Histogram with name: " << hName + "_" + theAppendix
          << " already existed at: " << ret.first->second;
    }
  } else {
    edm::LogWarning("MyCommonHistograms")
        << "[MyCommonHistoManager::Add] Warning: Histograms do not have a name. Please run SetName(std::string) first"
        << endl;
    return;
  }
...
void MyCommonHistoManager::Fill(const reco::Candidate *p, std::string theFullName) {
  idx = theCategories.find(theFullName);
  if (idx != theCategories.end()) {
    myHistos.at(idx->second)->Fill(p);
  } else {
    edm::LogWarning("MyCommonHistograms")
        << "[MyCommonHistoManager::Fill] Histogram " << hName << " - " << theFullName << " not found." << endl;
  }

  return;
}

void MyCommonHistoManager::Fill(const reco::Candidate *p1,
  ...
  if (idx != theCategories.end()) {
    ...
    edm::LogWarning("MyCommonHistograms") << "[MyCommonHistoManager::Fill] Histogram " << hName << " - "
                                          << theFullName + "_" + theName2 << " not found." << endl;
  }
...
MyCommonHistograms::MyCommonHistograms(std::string theFullName) {
  ...
  if (hName.find("Jpsi") != std::string::npos || hName.find("Upsilon") != std::string::npos ||
...
void MyCommonHistograms::BookParticleHistos() {
  ...
  hMass = new TH1F(("h" + hName + "_mass").c_str(),
                   ...
                   theMassBinning->GetNbins(),
                   theMassBinning->GetMinVal(),
                   theMassBinning->GetMaxVal());

  hE = new TH1F(("h" + hName + "_E").c_str(),
                ("Energy of " + hLabel + ";E (" + hLabel + ") [GeV];counts").c_str(),
                theEBinning->GetNbins(),
                theEBinning->GetMinVal(),
                theEBinning->GetMaxVal());
  hPt = new TH1F(("h" + hName + "_pt").c_str(),
                 ("Transverse momentum of " + hLabel + ";p_{T} (" + hLabel + ") [GeV/c];counts").c_str(),
                 thePtBinning->GetNbins(),
                 thePtBinning->GetMinVal(),
                 thePtBinning->GetMaxVal());
  hPhi = new TH1F(("h" + hName + "_phi").c_str(),
                  ("Aziumuthal angle of " + hLabel + ";#phi (" + hLabel + ") [rad];counts").c_str(),
                  thePhiBinning->GetNbins(),
                  thePhiBinning->GetMinVal(),
                  thePhiBinning->GetMaxVal());
  if (useRapidity) {
    hEta = new TH1F(("h" + hName + "_eta").c_str(),
                    ("Rapidity of " + hLabel + ";y (" + hLabel + ");counts").c_str(),
                    theEtaBinning->GetNbins(),
                    theEtaBinning->GetMinVal(),
                    theEtaBinning->GetMaxVal());
    hE_Eta =
        new TH2F(("h" + hName + "_E_eta").c_str(),
                 ...
                 theEBinning->GetMaxVal());
    hPt_Eta = new TH2F(
        ("h" + hName + "_pt_eta").c_str(),
        ("Transverse momentum vs. rapidity of " + hLabel + ";y (" + hLabel + ");p_{T} (" + hLabel + ") [GeV/c];counts")
            .c_str(),
        theEtaBinning->GetNbins(),
        theEtaBinning->GetMinVal(),
        theEtaBinning->GetMaxVal(),
    ...
    hPhi_Eta = new TH2F(
        ...
        thePhiBinning->GetMaxVal());
    hEta_Mass = new TH2F(
        ("h" + hName + "_eta_mass").c_str(),
        ("Rapidity vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel + ") [GeV/c^{2}];y (" + hLabel + ");counts")
            .c_str(),
        theMassBinning->GetNbins(),
        theMassBinning->GetMinVal(),
        theMassBinning->GetMaxVal(),
    ...
    hPt_Eta = new TH2F(("h" + hName + "_pt_eta").c_str(),
                       ("Transverse momentum vs. pseudo-rapidity of " + hLabel + ";#eta (" + hLabel + ");p_{T} (" +
                        hLabel + ") [GeV/c];counts")
                           .c_str(),
                       theEtaBinning->GetNbins(),
                       theEtaBinning->GetMinVal(),
    ...
    hPhi_Eta = new TH2F(("h" + hName + "_phi_eta").c_str(),
                        ("Azimuthal angle vs. pseudo-rapidity of " + hLabel + ";#eta (" + hLabel + ");#phi (" + hLabel +
                         ") [rad];counts")
                            .c_str(),
                        theEtaBinning->GetNbins(),
                        theEtaBinning->GetMinVal(),
    ...
    hEta_Mass = new TH2F(("h" + hName + "_eta_mass").c_str(),
                         ("Pseudo-rapidity vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel +
                          ") [GeV/c^{2}];#eta (" + hLabel + ");counts")
                             .c_str(),
                         theMassBinning->GetNbins(),
                         theMassBinning->GetMinVal(),
                         theMassBinning->GetMaxVal(),
  ...
  hE_Phi = new TH2F(
      ("h" + hName + "_E_phi").c_str(),
      ("Energy vs. azimuthal angle of " + hLabel + ";#phi (" + hLabel + ") [rad];E (" + hLabel + ") [GeV];counts")
          .c_str(),
      thePhiBinning->GetNbins(),
      thePhiBinning->GetMinVal(),
      thePhiBinning->GetMaxVal(),
  ...
  hPt_Phi = new TH2F(("h" + hName + "_pt_phi").c_str(),
                     ("Transverse momentum vs. azimuthal angle of " + hLabel + ";#phi (" + hLabel + ") [rad];p_{T} (" +
                      hLabel + ") [GeV/c];counts")
                         .c_str(),
                     thePhiBinning->GetNbins(),
                     thePhiBinning->GetMinVal(),
  ...
  hPhi_Mass = new TH2F(("h" + hName + "_phi_mass").c_str(),
                       ("Azimuthal angle vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel +
                        ") [GeV/c^{2}];#phi (" + hLabel + ") [rad];counts")
                           .c_str(),
                       thePhiBinning->GetNbins(),
                       thePhiBinning->GetMinVal(),
                       thePhiBinning->GetMaxVal(),
  ...
  hE_Mass = new TH2F(("h" + hName + "_E_mass").c_str(),
                     ("Energy vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel + ") [GeV/c^{2}];E (" + hLabel +
                      ") [GeV];counts")
                         .c_str(),
                     theMassBinning->GetNbins(),
                     theMassBinning->GetMinVal(),
  ...
  hPt_Mass = new TH2F(("h" + hName + "_pt_mass").c_str(),
                      ("Transverse momentum vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel +
                       ") [GeV/c^{2}];p_{T} (" + hLabel + ") [GeV/c];counts")
                          .c_str(),
                      theMassBinning->GetNbins(),
                      theMassBinning->GetMinVal(),
                      theMassBinning->GetMaxVal(),
  ...
  hCent_Mass = new TH2F(
      ("h" + hName + "_cent_mass").c_str(),
      ("Centrality vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel + ") [GeV/c^{2}];Centrality [%];counts")
          .c_str(),
      theMassBinning->GetNbins(),
      theMassBinning->GetMinVal(),
      theMassBinning->GetMaxVal(),
      theCentBinning->GetNbins(),
      theCentBinning->GetMinVal(),
      theCentBinning->GetMaxVal());

  hCtau_Mass = new TH2F(
      ("h" + hName + "_ctau_mass").c_str(),
      ("c#tau vs. invariant mass of " + hLabel + ";m_{inv} (" + hLabel + ") [GeV/c^{2}];l_{" + hLabel + "} [mm];counts")
          .c_str(),
      theMassBinning->GetNbins(),
      theMassBinning->GetMinVal(),
      theMassBinning->GetMaxVal(),
      theCtauBinning->GetNbins(),
      theCtauBinning->GetMinVal(),
      theCtauBinning->GetMaxVal());
...
void MyCommonHistograms::BookParticleHistos(std::string hName2) {
  edm::LogInfo("MyCommonHistograms") << "[MyCommonHistograms::BookParticleHistos] " << hName;
...
void MyCommonHistograms::Fill(const reco::Candidate *p1, const reco::Candidate *p2, std::string hName2) {
  if (!booked2ParticleHistos)
    BookParticleHistos(hName2);
...
void MyCommonHistograms::Write(TFile *outf) {
  ...
  if (booked1ParticleHistos) {
    if (outf->FindKey(hName.c_str()) == nullptr)
...
std::string MyCommonHistograms::MakeLabel(std::string name) {
  std::string label;
