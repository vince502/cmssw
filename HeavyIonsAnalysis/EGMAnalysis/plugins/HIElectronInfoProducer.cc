...
//Working point: WP95
if(wp==0) {
  if(isEB) cut=-1.4759457106302918e-06*pow(cen,3)+0.0002549248870342906*pow(cen,2)+-0.01505841250057598*cen+0.49812702686679877;
  else     cut=-1.0029999817518148e-06*pow(cen,3)+0.00021237989319995343*pow(cen,2)+-0.018242925240307368*cen+0.8864788386101833;
}
//Working point: WP90
if(wp==1) {
  if(isEB) cut=-8.017038078686935e-07*pow(cen,3)+0.0001398279725697041*pow(cen,2)+-0.008225559123563741*cen+0.23666484622910894;
  else     cut=-1.952326794536896e-06*pow(cen,3)+0.0003744387849551835*pow(cen,2)+-0.025330603452544884*cen+0.777274396638155;
}
//Working point: WP85
if(wp==2) {
  if(isEB) cut=-4.0769610898861085e-07*pow(cen,3)+7.205307414566228e-05*pow(cen,2)+-0.004281409019080686*cen+0.12063600016712422;
  else     cut=-2.2750481944177227e-06*pow(cen,3)+0.0004163045093827872*pow(cen,2)+-0.025559229487206903*cen+0.6416422347565578;
}
//Working point: WP80
if(wp==3) {
  if(isEB) cut=-2.3306302184891391e-07*pow(cen,3)+4.0878621787159674e-05*pow(cen,2)+-0.0023940488176704237*cen+0.06668280047972538;
  else     cut=-2.0270348969895824e-06*pow(cen,3)+0.0003652079074130704*pow(cen,2)+-0.021656673636701006*cen+0.4995808721720696;
}
  return mva < cut;
}
