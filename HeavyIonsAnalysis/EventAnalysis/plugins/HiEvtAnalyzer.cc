...
void HiEvtAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // Small-system MiniAOD runs with centrality disabled. Keep the always-booked
  // centrality and HF branches deterministic instead of exposing uninitialized
  // memory in the common event tree.
  if (!doCentrality_) {
    hiBin = -1;
    hiNpix = hiNpixelTracks = hiNtracks = -1;
    hiNtracksPtCut = hiNtracksEtaCut = hiNtracksEtaPtCut = -1;
    hiNpixPlus = hiNpixMinus = -1;
    hiNpixelTracksPlus = hiNpixelTracksMinus = -1;
    hiHF = hiHFplus = hiHFminus = -1.f;
    hiHFplusEta4 = hiHFminusEta4 = -1.f;
    hiHFhit = hiHFhitPlus = hiHFhitMinus = -1.f;
    hiHFECut = hiHFECutPlus = hiHFECutMinus = -1.f;
    hiEB = hiET = hiEE = hiEEplus = hiEEminus = -1.f;
    hiZDC = hiZDCplus = hiZDCminus = -1.f;
    hiHF_pf = hiHFE_pf = hiHF_pfha = hiHF_pfem = -1.f;
    hiHFPlus_pf = hiHFEPlus_pf = hiHFPlus_pfha = hiHFPlus_pfem = -1.f;
    hiHFMinus_pf = hiHFEMinus_pf = hiHFMinus_pfha = hiHFMinus_pfem = -1.f;
    nCountsHF_pf = nCountsHFPlus_pf = nCountsHFMinus_pf = -1;
    for (int i = 0; i < NHFLEAD; ++i) {
      hiHF_pfle[i] = hiHFPlus_pfle[i] = hiHFMinus_pfle[i] = -1.f;
    }
  }
